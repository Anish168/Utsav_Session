...
node *createbstusingsortedarray(int *arr, int start, int end)
{
    // base case
    if (start > end)
    {
    ...
    root->left = createbstusingsortedarray(arr, start, mid - 1); // recruisn mujhe lst dega
    root->right = createbstusingsortedarray(arr, mid + 1, end);  // recruisn mujhe rst dega
    return root;
}
int main()
{
    ...
    createbstusingsortedarray(arr, 0, n - 1);
    return 0;
}