...
node *deletion(node *root, int key)
{
    ...
    // agar key mera lst m exit krta h
    else if (root->data < key)
    {
        root->left = deletion(root->left, key);
    }
    // agar key mera rst m exit krta h
    else if (root->data > key)
    {
    ...
    {
        // mujhe root->data ko del krna h.
        // case 1: - lst & rst exit nhi krta
        if (root->left == NULL && root->right == NULL)
        {
            delete root;
            root = NULL;
        ...
        else
        {
            ...
            {
                replace = replace->left; // apko min m mill jyga rst ka
            }
